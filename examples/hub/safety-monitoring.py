...
Loads the SafetyMonitor, SafetyActionMapper, and ActionPrinter flows from a
Hub module chosen by `RETRIEVER_HUB_SAFETY_MONITOR_MODULE`. `SafetyScenario`
...
MODULE_REF = os.environ.get('RETRIEVER_HUB_SAFETY_MONITOR_MODULE', '').strip()
if not MODULE_REF:
    raise SystemExit(
        'Set RETRIEVER_HUB_SAFETY_MONITOR_MODULE=org/module before running this example.'
    )

sm = hub.use(MODULE_REF)
