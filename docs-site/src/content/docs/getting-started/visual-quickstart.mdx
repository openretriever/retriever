...
This is a source-checkout demo target. The planned runtime package target is `retriever-core`, but examples, Rerun, graph rendering, and tutorial assets live in the repository environment.

<div class="notebook-takeaway"><strong>Success criterion:</strong> the mock command runs without a camera, GUI, Rerun viewer, or robot backend. If you only want to read first, jump to the representative output and compare it later.</div>
...
| Rerun viewer does not open | `pixi run python -m examples.tutorial.b_ir_and_execution.06_dora_perception --backend in-process --camera-mode real --visualize stdout --duration 10` | Keeps runtime verification separate from viewer setup. |
