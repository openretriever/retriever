...
<p class="nb-actions">
  <a class="nb-download" href="/notebooks/flow-fundamentals.ipynb" download>⬇&nbsp; Download&nbsp;.ipynb</a>
  <a class="nb-download nb-colab" href="https://colab.research.google.com/github/openretriever/retriever/blob/main/docs-site/public/notebooks/flow-fundamentals.ipynb" target="_blank" rel="noopener">▶&nbsp; Open in Colab</a>
</p>
...
<p class="notebook-takeaway"><strong>Takeaway:</strong> the Flow owns local computation; the graph owns <em>when</em> it wakes and <em>which</em> upstream record it consumes. <code>src.then(dbl, sync=Latest())</code> is the fluent composition form; <code>connect(src, dbl, sync=Latest())</code> is the explicit equivalent. Next: <a href="/concepts/time-and-sync/">Time and Sync</a> shows how clocks and sync policies build the input passed to <code>step()</code>.</p>
