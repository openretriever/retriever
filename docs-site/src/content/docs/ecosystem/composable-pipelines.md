...
```bash
retriever run composable-pipelines
```
...
`Pipeline.visualize(...)` and `IR.visualize(...)` keep wrapped-pipeline context: a nested `build_pipeline_flow(...)` stage renders as a grouped box around its lowered inner flows, with the pipeline name and surfaced port bindings. Render the tutorial graph with `retriever run graph-composable`.
