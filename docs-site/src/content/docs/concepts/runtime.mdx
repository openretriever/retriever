...
- Equal timestamps require a fixed tie order. Release `0.0.2` preserves insertion order for ties; use monotonic timestamps or sequence keys when multiple producers can emit at exactly the same time.
