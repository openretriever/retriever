...
**The contract, precisely.** Fix a graph with deterministic `Flow.step()` bodies, deterministic clocks and `sync` policies, fixed initial states, strictly causal cycles, and the same timestamped external input history; replay any stochastic model calls from logged seeds or logged outputs. Then the output history is uniquely determined — independent of runtime scheduling. Two implementation details do the work: each edge samples only events timestamped *strictly before* the consumer's tick (ties need an explicit, deterministic tie rule), and execution order is pinned by the IR's topology (a stable ordering that a saved IR is re-validated against on load). We've verified this empirically: identical output traces across processes and Python hash seeds on cyclic multi-rate graphs, plus the `verify-incident-replay` gate asserting live-vs-replay equality.

Boundaries worth knowing:

- `step(now=...)` and replaying a recording are exact. `step(dt=...)` keeps all *deltas* deterministic but anchors the logical clock at the first step's wall time.
- Determinism concerns the discrete event path, not bitwise GPU numerics.
- A *live* async run (`multiprocessing`, `dora`) does not replay its scheduling — arrival timing is real. Record the run; the recording replays exactly.
