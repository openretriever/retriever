...
```bash
./scripts/retriever run ir-validation   # prints the full IR as JSON
```
...
```bash
./scripts/retriever run stepper   # the shipped in-process stepper, with a --fail-at breakpoint
```
...
```bash
./scripts/retriever run multirate         # multiprocessing
./scripts/retriever run webcam-dora   # dora
```
...
```bash
./scripts/retriever run record-replay     # record a run, then replay it step-for-step
./scripts/retriever run incident-replay   # replay a captured incident and diff the diagnosis
```
