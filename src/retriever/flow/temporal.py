"""
FlowHandle for binding flows to clocks and creating connections.

A FlowHandle represents a flow bound to a clock, forming a node
...
class TemporalFlow:
    ...
    def then(
        ...
        Args:
            ...
            sync: Adapter for queue sampling (defaults to Latest())
        ...
        if ctx is not None:
            ...
            #       a >> b
            #
            # and also allows continuing wiring outside the context because the handles
            # will be tagged with `handle.pipeline = pipe`.
            from retriever.flow.pipeline import Pipeline
            if isinstance(ctx, Pipeline):
                ...
                ctx.connect(self, next, map=map, sync=sync, qsize=qsize)
                return next
            ...
            ctx.register_connection(src=self, dst=next, map=map, sync=sync, qsize=qsize)
            return next
        ...
        pipeline.connect(self, next, map=map, sync=sync, qsize=qsize)
