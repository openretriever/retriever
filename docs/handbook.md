...
- **Pixi** manages a full environment from `pixi.toml` and installs the PyPI portion via `uv` internally. The planned public distribution name is `retriever-core`, while the import package remains `retriever`.
