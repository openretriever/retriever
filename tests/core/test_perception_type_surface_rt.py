...
def test_perception_package_exports_canonical_surface() -> None:
    frame = Image2D(data=np.zeros((8, 10, 3), dtype=np.uint8), encoding='rgb8', frame_index=3)
...
def test_perception_types_are_flow_compatible() -> None:
    assert is_flow_io(Image2D)
    assert is_flow_io(DetectionBatch)
    assert is_flow_io(SegmentationMask2D)
    assert is_flow_io(PointTarget2D)
...
def test_resolve_schema_ref_supports_perception_types() -> None:
    msg = DetectionBatch(
        detections=(
            Detection2D(label='red', bbox=BBox2D(x=1.0, y=2.0, width=3.0, height=4.0), confidence=0.9, pixel_count=12),
        ),
...
def test_validate_image_and_compressed_image() -> None:
    validate_image2d(Image2D(data=np.zeros((4, 5), dtype=np.uint8), encoding='mono8', frame_index=0))
    validate_compressed_image2d(CompressedImage2D(data=b'abc', format='jpeg', frame_index=1))
    ...
    with pytest.raises(ValueError):
        validate_image2d(Image2D(data=np.zeros((4, 5), dtype=np.uint8), encoding='mono8', frame_index=-1))
...
def test_validate_video_pointcloud_and_mask() -> None:
    ...
    validate_segmentation_mask2d(SegmentationMask2D(mask=np.zeros((5, 6), dtype=np.int32), label_map={0: 'bg'}, frame_index=4))
...
def test_detection_batch_and_point_target_frame_index_validation() -> None:
    validate_detection_batch(DetectionBatch(frame_index=2))
    validate_point_target2d(PointTarget2D(label='red', x_norm=0.2, y_norm=0.7, confidence=0.8, frame_index=5))

    with pytest.raises(ValueError):
        validate_detection_batch(DetectionBatch(frame_index=-1))
    with pytest.raises(ValueError):
        validate_point_target2d(PointTarget2D(frame_index=-1))


def test_point_target2d_is_simple_optional_boundary() -> None:
    target = PointTarget2D(label='red', x_norm=0.2, y_norm=0.7, confidence=0.8, frame_index=5)
    assert target.label == 'red'
    assert target.frame_index == 5
